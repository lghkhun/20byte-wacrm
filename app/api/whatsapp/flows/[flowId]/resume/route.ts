...
import { resumeWhatsAppFlow } from "@/server/services/whatsappCampaignService";
...
export async function POST(
  ...
  try {
    ...
    const item = await resumeWhatsAppFlow(auth.session.userId, orgId, params.flowId);
    return successResponse({ item }, 200);
  } catch (error) {
    if (error instanceof ServiceError) {
      return errorResponse(error.status, error.code, error.message);
    }
    return errorResponse(500, "WHATSAPP_FLOW_RESUME_FAILED", "Failed to resume WhatsApp flow.");
  }
}
