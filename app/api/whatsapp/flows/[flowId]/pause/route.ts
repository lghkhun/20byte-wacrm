...
import { pauseWhatsAppFlow } from "@/server/services/whatsappCampaignService";
...
export async function POST(
  ...
  try {
    ...
    const item = await pauseWhatsAppFlow(auth.session.userId, orgId, params.flowId);
    return successResponse({ item }, 200);
  } catch (error) {
    if (error instanceof ServiceError) {
      return errorResponse(error.status, error.code, error.message);
    }
    return errorResponse(500, "WHATSAPP_FLOW_PAUSE_FAILED", "Failed to pause WhatsApp flow.");
  }
}
